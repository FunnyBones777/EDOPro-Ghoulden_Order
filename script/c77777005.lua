...
function s.initial_effect(c)
	--link summon
	Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x420a),3,3)
	c:EnableReviveLimit()
	--Special Summon "Ghoulden Order" monsters from your GY
	...
	c:RegisterEffect(e1)
	--Special Summon 1 "Ghoulden Order" monster from your GY
...
function s.spfilter(c,e,tp,zone)
	return c:IsLevelBelow(4) and c:IsSetCard(0x0420) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP,tp,zone)
end
...
function s.filter(c,e,tp)
	return c:IsSetCard(0x0420) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP,tp)
end
